...
export function HistoryClient({
  ...
  const { user } = useAuth();
  const [activeTab, setActiveTab] = React.useState<"all" | "sent" | "received">(
    "all",
  );
  const {
  ...
  const [storedWalletAddress, setStoredWalletAddress] = React.useState<
    string | null
  >(null);
  ...
  const [selectedTx, setSelectedTx] =
    React.useState<EnrichedTransaction | null>(null);
  ...
  const effectiveAddress =
    storedWalletAddress || (isConnected ? connectedAddress : null);
    ...
    async function load() {
      if (hasWallet && effectiveAddress) {
        ...
        try {
          ...
          const orgName =
            profile?.organization_name || profile?.full_name || "My Business";
  ...
  const transactions = txs.filter((t) => {
    ...
    if (activeTab === "received")
      return t.recipientAddress === effectiveAddress;
    return true;
  });
      ...
      <div className="space-y-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            A complete record of all your Bitcoin & STX transfers.
          </p>
        </div>
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="flex items-center gap-1 p-1 bg-accent/20 rounded-2xl w-fit"
        variants={itemVariants}
      >
            ...
            <DataTable
              ...
              columns={[
                ...
                },
                {
                  ...
                  accessorKey: (tx: EnrichedTransaction) => (
                    <div className="flex flex-col">
                      <span className="font-semibold text-[10px] md:text-sm truncate max-w-[80px] md:max-w-none">
                        {tx.recipientName}
                      </span>
                    ...
                    <div className="flex flex-col items-end md:items-start">
                      <span
                        className={cn(
                          ...
                          tx.senderAddress === connectedAddress
                            ? ""
                            : "text-green-600",
                        )}
                      >
