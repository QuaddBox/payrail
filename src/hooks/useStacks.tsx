...
export function useStacks() {
  ...
  const userData =
    isBrowser && userSession && userSession.isUserSignedIn()
  ...
  const connectWallet = useCallback(() => {
    ...
    try {
      ...
      if (typeof connectFn === "function") {
        ...
        console.error(
          "Could not find Stacks connection function in @stacks/connect",
        );
        showNotification(
          "error",
          "Wallet connection error. Please ensure you have a Stacks wallet installed.",
        );
      }
    } catch (error: any) {
  ...
  const createOrganization = useCallback(
    async (name: string, onFinish?: (data: any) => void) => {
      return StacksConnect.openContractCall({
        contractAddress: CONTRACT_ADDRESS,
        contractName: CONTRACT_NAME,
  ...
  // Supports delegate execution via optional onBehalfOf parameter
  const executeBatchPayroll = useCallback(
    async (
      recipients: { address: string; amountSTX: number }[],
      periodRef: string,
      onFinish?: (data: any) => void,
      onBehalfOf?: string, // Owner address if caller is a delegate
    ) => {
      ...
      const totalUSTX = recipients.reduce(
        (sum, r) => sum + Math.floor(r.amountSTX * 1_000_000),
        0,
      );
      const senderAddress =
        userData?.profile?.stxAddress?.testnet ||
      ...
      // Build function args - include on-behalf-of as optional principal
      const functionArgs = [
        listCV(recipientList),
        stringAsciiCV(periodRef.slice(0, 32)), // Max 32 chars
        onBehalfOf ? someCV(principalCV(onBehalfOf)) : noneCV(),
      ];
      ...
      return StacksConnect.openContractCall({
        ...
        onFinish: (data: any) => {
          ...
          showNotification(
            "success",
            `Batch payroll broadcasted! ${recipients.length} recipients`,
          );
          if (onFinish) onFinish(data);
        },
        onCancel: () => {
    ...
    async (businessAddress: string) => {
      try {
        ...
        if (response && response.type === ClarityType.OptionalSome) {
          ...
          const orgIdCV =
            tupleData && typeof tupleData === "object"
              ? tupleData["org-id"]
              : null;
  ...
  const authorizeDelegate = useCallback(
    async (delegateAddress: string, onFinish?: (data: any) => void) => {
      return StacksConnect.openContractCall({
        contractAddress: CONTRACT_ADDRESS,
        contractName: CONTRACT_NAME,
        functionName: "authorize-delegate",
        functionArgs: [principalCV(delegateAddress)],
        network,
        anchorMode: AnchorMode.Any,
        postConditionMode: PostConditionMode.Deny,
        onFinish: (data: any) => {
          console.log("Authorize delegate finished:", data);
          showNotification("success", "Delegate authorization broadcasted!");
          if (onFinish) onFinish(data);
        },
        onCancel: () => {
          console.log("Authorize delegate cancelled");
          showNotification("info", "Authorization cancelled");
        },
      });
    },
    [network, showNotification],
  ...
  const revokeDelegate = useCallback(
    async (delegateAddress: string, onFinish?: (data: any) => void) => {
      return StacksConnect.openContractCall({
        contractAddress: CONTRACT_ADDRESS,
        contractName: CONTRACT_NAME,
        functionName: "revoke-delegate",
        functionArgs: [principalCV(delegateAddress)],
        network,
        anchorMode: AnchorMode.Any,
        postConditionMode: PostConditionMode.Deny,
        onFinish: (data: any) => {
          console.log("Revoke delegate finished:", data);
          showNotification("success", "Delegate revocation broadcasted!");
          if (onFinish) onFinish(data);
        },
        onCancel: () => {
          console.log("Revoke delegate cancelled");
          showNotification("info", "Revocation cancelled");
        },
      });
    },
    [network, showNotification],
  ...
  // Check if a wallet is authorized to act on behalf of an owner
  const checkIsAuthorized = useCallback(
    async (ownerAddress: string, callerAddress: string) => {
      try {
        const response = await fetchCallReadOnlyFunction({
          contractAddress: CONTRACT_ADDRESS,
          contractName: CONTRACT_NAME,
          functionName: "is-authorized",
          functionArgs: [principalCV(ownerAddress), principalCV(callerAddress)],
          network,
          senderAddress: callerAddress,
        });

        // Response is a boolean
        if (response && response.type === ClarityType.BoolTrue) {
          return true;
        }
        return false;
      } catch (e) {
  ...
  const getSTXBalance = useCallback(
    async (stxAddress: string) => {
      try {
        const isMainnet =
          CONTRACT_ADDRESS.startsWith("S") &&
          !CONTRACT_ADDRESS.startsWith("ST");
        const apiUrl = isMainnet
          ? "https://api.mainnet.hiro.so"
          : "https://api.testnet.hiro.so";
        const response = await fetch(
          `${apiUrl}/extended/v1/address/${stxAddress}/balances`,
        );
    ...
    async (stxAddress: string) => {
      try {
        ...
        const [confirmedRes, mempoolRes] = await Promise.all([
          fetch(
            `${apiUrl}/extended/v1/address/${stxAddress}/transactions?limit=50`,
          ),
  ...
  const getTransactionEvents = useCallback(async (txId: string) => {
    try {
      const isMainnet =
        CONTRACT_ADDRESS.startsWith("S") && !CONTRACT_ADDRESS.startsWith("ST");
      const apiUrl = isMainnet
        ? "https://api.mainnet.hiro.so"
        : "https://api.testnet.hiro.so";
      ...
      console.log(
        "[getTransactionEvents] Raw events:",
        tx.events?.map((e: any) => e.event_type),
      );
      ...
      for (const event of tx.events || []) {
        ...
        // Handle fungible_token_asset for STX (another possible format)
          ...
          event.asset?.asset_id?.includes("stx")
        ) {
          stxTransfers.push({
            sender: event.asset.sender || "",
            recipient: event.asset.recipient || "",
