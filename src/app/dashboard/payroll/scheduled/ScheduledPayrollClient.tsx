...
export default function ScheduledPayrollPage({
  ...
  React.useEffect(() => {
    async function fetchOwnerWallet() {
      if (user) {
        ...
        const storedOwnerWallet = profile?.wallet_address || null;
        setOwnerWallet(storedOwnerWallet);

        // Check if connected wallet is different from owner (delegate)
        if (myAddress && storedOwnerWallet) {
          const isDifferentWallet =
            myAddress.toLowerCase() !== storedOwnerWallet.toLowerCase();
          setIsDelegate(isDifferentWallet);
        }
      }
    }
    fetchOwnerWallet();
  }, [user, supabase, myAddress]);
  ...
  const handleRunPayroll = async (schedule: PayrollSchedule) => {
    ...
    if (currentPrice <= 0) {
      showNotification(
        "error",
        "Price Not Available",
        "Unable to fetch STX price. Please wait a moment and try again.",
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        "[BatchPayroll] Full schedule data:",
      ...
      // Determine if we need to pass onBehalfOf (for delegate execution)
      const onBehalfOfAddress =
        isDelegate && ownerWallet ? ownerWallet : undefined;

      console.log("[BatchPayroll] Executing as delegate:", isDelegate);
      console.log("[BatchPayroll] onBehalfOf:", onBehalfOfAddress);

      // Execute batch payroll (pass onBehalfOfAddress for delegate execution)
      await executeBatchPayroll(
        recipients,
        periodRef,
        async (data: any) => {
          const txId = data.txId || "";
          const totalAmount = schedule.payroll_schedule_items.reduce(
            (sum, i) => sum + i.amount,
            0,
          );
          ...
          if (!verification.success) {
            ...
            console.error(
              `[BatchPayroll] Transaction FAILED: ${verification.error}`,
            );
            showNotification(
              "error",
              "Transaction Failed",
              verification.error ||
                "The on-chain transaction failed. No payments were made.",
            );
          ...
          console.log(
            `[BatchPayroll] Transaction CONFIRMED! Sending emails...`,
          );
          ...
          console.log(
            `[BatchPayroll] Sending emails to ${schedule.payroll_schedule_items.length} recipients`,
          );
          for (const item of schedule.payroll_schedule_items) {
            ...
            console.log(
              `[BatchPayroll] Sending email to ${teamMember.name} (${teamMember.email || "no email"})`,
            );

            if (!teamMember.email) {
              console.log(
                `[BatchPayroll] Skipping ${teamMember.name} - no email address`,
              );
              continue;
            }
            ...
            try {
              ...
              console.log(
                `[BatchPayroll] Email result for ${teamMember.name}:`,
                result,
              );
            } catch (emailErr) {
              console.error(
                `[BatchPayroll] Email error for ${teamMember.name}:`,
                emailErr,
              );
            }
          }
          ...
          showNotification(
            "success",
            "Payroll Confirmed!",
            `${recipients.length} payments confirmed on-chain.`,
          );
          router.refresh();
          loadSchedules();
        },
  ...
  // Calculate due schedules - compare full timestamps, not just dates
  const now = new Date();
  const dueSchedules = schedules.filter((s) => {
    if (!s.next_run_at) return false;
    const nextRunTime = new Date(s.next_run_at);
    // Schedule is due if the next_run_at time has PASSED (not just same day)
    return (s.status === "ready" || s.status === "draft") && nextRunTime <= now;
  });
  ...
  const handlePayAllDue = async () => {
    ...
    try {
      ...
      showNotification(
        "error",
        "Error",
        err.message || "An error occurred while processing payrolls",
      );
    } finally {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      {/* Due Count Alert */}
      {dueCount > 0 && (
        <div className="bg-linear-to-r from-orange-500/10 to-red-500/10 border border-orange-500/30 rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="bg-red-500 text-white text-sm font-bold px-3 py-1.5 rounded-full animate-pulse shrink-0">
              {dueCount} Due
            </div>
            <span className="text-sm text-muted-foreground">
              {dueCount === 1 ? "1 payroll is" : `${dueCount} payrolls are`}{" "}
              ready to be paid
            </span>
          </div>
          <Button
            className="w-full sm:w-auto rounded-xl font-bold bg-linear-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 shadow-lg"
      ...
      <div className="grid grid-cols-1 gap-6">
        {isLoading ? (
          ...
          schedules.map((schedule) => {
            ...
            const isDue = schedule.next_run_at
            ...
            return (
              <Card
                key={schedule.id}
                className="border-none shadow-xl shadow-black/5 overflow-hidden group hover:ring-2 hover:ring-primary/20 transition-all duration-300 bg-card/50 backdrop-blur-sm"
              >
                <CardContent className="p-0">
                  <div className="flex flex-col">
                    {/* Top Section - Header-like */}
                    <div className="p-6 pb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-white/5">
                      <div className="flex items-center gap-4">
                        <div className="h-14 w-14 rounded-2xl bg-linear-to-br from-primary/20 to-primary/5 flex items-center justify-center text-primary shrink-0 shadow-inner">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent
                            align="end"
                            className="w-48 p-1.5 rounded-2xl bg-popover/90 backdrop-blur-md border border-white/10 shadow-2xl"
                          >
                            <DropdownMenuItem
                              className="text-red-500 focus:text-red-500 focus:bg-red-500/10 rounded-xl px-3 py-2.5"
                              onClick={() =>
                                handleDeleteRequest(schedule.id, schedule.name)
                              }
                            >
                    ...
                    <div className="px-6 py-6 grid grid-cols-2 md:grid-cols-4 gap-6 bg-linear-to-b from-transparent to-accent/5">
                    ...
                    <div className="p-6 pt-4 flex flex-col md:flex-row items-center justify-between gap-6 bg-accent/10 border-t border-white/5">
                      <div className="w-full md:w-auto">
                        <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 mb-3">
                          Recent Recipients
                        </p>
                        <div className="flex flex-wrap items-center gap-2">
                          {schedule.payroll_schedule_items
                            .slice(0, 3)
                            .map((item) => (
                              <div
                                key={item.id}
                                className="group/item flex items-center gap-2 bg-background/40 hover:bg-background/80 border border-white/3 rounded-xl px-3 py-2 transition-all cursor-default"
                              >
          ...
          <Card className="shadow-sm bg-accent/5 border-2 border-dashed border-accent/20">
            <CardContent className="p-12 flex flex-col items-center text-center space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-muted-foreground max-w-xs mx-auto text-sm">
                  Create a recurring payroll schedule to automate your team
                  payments.
                </p>
              </div>
              <Button
